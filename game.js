...
(function() {
  ...
  // increase is 9×, so the maximum number of items (fruit/ammo) is
  // scaled up proportionally.  Items spawn rate is 1.5× faster than
  // before.  These constants determine the playfield size and spawn
  // frequency.
  ...
  // The number of ammo pickups allowed on the map simultaneously.  We
  // scale it to the same proportion as fruit so that the world
  // contains a balanced mix of items.
  const MAX_AMMO_COUNT = 40 * 9;
  ...
  class AmmoPickup {
    constructor() {
      ...
      // Radii scaled roughly with the ammo amount for visual variety.
      const radii = {1: 0.15, 3: 0.25, 5: 0.35, 10: 0.5};
      this.radius = radii[this.amount] || 0.2;
      // Position within world boundaries
      this.x = Math.random() * (WORLD_WIDTH - 2 * this.radius) + this.radius;
      this.y = Math.random() * (WORLD_HEIGHT - 2 * this.radius) + this.radius;
      // Colours chosen to differentiate ammo from fruit.  Darker hues
      // emphasise their distinct role.
      const colours = {1: '#4A90E2', 3: '#9B51E0', 5: '#2D9CDB', 10: '#BB6BD9'};
      this.color = colours[this.amount] || '#4A90E2';
    }
  }
  ...
  function spawnItem() {
    ...
    let item;
    if (spawnType === 'fruit') item = new Fruit();
    else item = new AmmoPickup();
    items.push(item);
  }
  ...
  function applyUpgrade(type) {
    ...
    switch (type) {
      case 'bulletSpeed':
        player.bulletSpeedMultiplier *= 1.2;
  ...
  function update() {
    if (gameRunning && player) {
      ...
      healthValue.textContent = 'Health: ' + Math.floor(player.health) + '/' + Math.floor(player.maxHealth);
      if (availablePts > 0) {
        // Build an instruction string showing each upgrade and its current level
        upgradeList.innerHTML =
          '<p><strong>Upgrade Options</strong> (press key to apply):</p>' +
          '<p>1: Bullet Speed (lvl ' + player.upgrades.bulletSpeed + ')</p>' +
          '<p>2: Bullet Range (lvl ' + player.upgrades.bulletRange + ')</p>' +
          '<p>3: Health (lvl ' + player.upgrades.health + ')</p>' +
          '<p>4: Speed (lvl ' + player.upgrades.speed + ')</p>';
      } else {
        upgradeList.innerHTML = '';
      }
    }
