...
(function() {
  ...
  // target practice bot.
  // Damage dealt to the target practice bot when hit by a bullet.  Each
  // hit removes this many health points.  The value has been
  // increased to 20 to make bullets more lethal, especially as
  // chasing bots gain additional health in later rounds.
  ...
  class TargetBot {
    ...
    update() {
      ...
      // Compute a chase vector toward the player
      let vx = player.x - this.x;
      let vy = player.y - this.y;
      // Also push away from the nearest obstacle to avoid collisions
      let nearest = null;
      let minDist = Infinity;
      for (const ob of obstacles) {
        let cx, cy;
      ...
      if (nearest) {
        // Add a small repulsion away from the obstacle centre
        vx += (this.x - nearest.cx) * 0.5;
        vy += (this.y - nearest.cy) * 0.5;
      }
      // If the chase vector is nearly zero (bot on top of player),
      // choose a random direction to keep moving
      ...
      // Detect if the bot has become stuck.  Compare its movement
      // between frames; if the displacement is extremely small, we
      // count the frame as stalled.  Once the stall timer exceeds
      // 15 seconds (900 frames at 60 FPS), respawn the bot on the
      // opposite side of the map relative to the player.  After
      // respawning, reset the stall counter.  The bot's health
      // persists across respawns.
      {
        // Determine how far we moved this frame.  We store prevX and
        // prevY in closure variables outside of update() since
        // update() runs many times per second.  If not present,
        // initialise them to the current position.  These
        // properties are added dynamically to the instance.
        ...
        this.prevX = this.x;
        this.prevY = this.y;
        // If moved less than a small fraction of speed, count as stalled
        if (movedDist < this.speed * 0.1) {
          this.stalledFrames++;
        } else {
          this.stalledFrames = 0;
        }
        const STUCK_FRAMES = 60 * 15; // 15 seconds
        if (this.stalledFrames >= STUCK_FRAMES) {
          // Respawn on opposite side of the map relative to the player.  If
          // the player is on the left half, spawn somewhere on the
          // right half (and vice versa).  Same for vertical axis.  Try
          // multiple attempts to avoid obstacles.  Keep health the same.
          let newX, newY;
          let attempts = 0;
          while (attempts < 50) {
            // Choose a region opposite to the player's position
