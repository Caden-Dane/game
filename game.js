...
(function() {
  ...
  const PROGRESS_BAR_FRAMES = 60 * 180;
  let progressFrame = 0;
  ...
  function update() {
    if (gameRunning && player) {
      ...
      // Update the three‑minute progress bar.  Increment the frame
      // counter until it reaches the threshold, then spawn an extra
      // enemy bot if we haven’t already.  Update the bar’s width
      // accordingly.
      ...
      } else if (!progressBotSpawned) {
        // Spawn an additional chasing bot once the bar fills
        targetBots.push(new TargetBot());
        progressBotSpawned = true;
      }
    }
