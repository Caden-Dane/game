...
(function() {
  ...
  function spawnItem() {
    ...
    let item;
    let attempts = 0;
    // Try multiple times to find a position that does not collide with
    // obstacles.  We increase the number of retries to improve the
    // likelihood of success in crowded maps.  If all attempts fail
    // we simply skip spawning this item to avoid placing it inside
    // an obstacle.
    while (attempts < 50) {
  ...
  class Bot {
    constructor(type) {
      ...
      // Mode tracks bot behaviour: 'normal' means chasing a target,
      // 'reversing' means moving away from its target after being stuck.
      this.mode = 'normal';
      // While reversing, reverseFrames counts how many frames the bot
      // has moved in the opposite direction.  reverseVector stores
      // the x and y components of the unit vector pointing away from
      // the current target.
    ...
    update() {
      ...
      // If the bot is currently reversing away from its target
      if (this.mode === 'reversing') {
        // Move in the stored reverse direction
        this.moveInDirection(this.reverseVector.x, this.reverseVector.y);
        this.reverseFrames++;
        // After moving in reverse for 3 seconds, choose a far target and return to normal mode
        if (this.reverseFrames >= threshold) {
          this.mode = 'normal';
          this.reverseFrames = 0;
          // Pick the 5th farthest item of the bot's type
          const newTarget = this.findFarthestTarget(5);
          if (newTarget) {
        ...
        if (touching) {
          this.contactFrames++;
          if (this.contactFrames >= threshold) {
            // Start reversing away from the current target
            this.mode = 'reversing';
            this.reverseFrames = 0;
            // Compute inverse direction from bot to target
            if (this.target) {
              const dx = this.x - this.target.x;
              const dy = this.y - this.target.y;
              const len = Math.hypot(dx, dy);
              if (len > 0) {
                this.reverseVector.x = dx / len;
                this.reverseVector.y = dy / len;
              } else {
                // Default to random direction if target coincides
                const angle = Math.random() * Math.PI * 2;
                this.reverseVector.x = Math.cos(angle);
                this.reverseVector.y = Math.sin(angle);
              }
            } else {
          ...
          // Not touching obstacle: reset contact tracker
          this.contactFrames = 0;
          this.redirectAttempts = 0;
        }
        // Check if bot collides with its target; if so, collect it
        if (this.target) {
