...
(function() {
  ...
  class TargetBot {
    ...
    update() {
      ...
      // We do not include the player's speed boost here so the bot
      // remains catchable even when the player is boosted.
      this.speed = (6 / 60) * player.speedMultiplier;
      // Compute a flee vector away from the player
      let vx = this.x - player.x;
      let vy = this.y - player.y;
      // Also flee from the nearest obstacle (use centre of obstacle)
      let nearest = null;
      let minDist = Infinity;
      for (const ob of obstacles) {
        let cx, cy;
      ...
      // If the resulting vector is very small (we’re on top of player),
  ...
  function startGame() {
    ...
    // Spawn the target practice bot at the beginning of the game
    targetBot = new TargetBot();
  }
  ...
  function update() {
    if (gameRunning && player) {
      ...
      for (let i = bullets.length - 1; i >= 0; i--) {
        const b = bullets[i];
        b.update();
        // Check collision with the target practice bot.  If a bullet
        // hits the bot, reduce its health and remove the bullet.  We
        // skip further processing for this bullet.
        if (targetBot && targetBot.health > 0) {
          const distBot = Math.hypot(b.x - targetBot.x, b.y - targetBot.y);
          if (distBot < b.radius + targetBot.radius) {
            targetBot.health -= BULLET_DAMAGE;
            // Ensure health doesn’t drop below zero
            if (targetBot.health < 0) targetBot.health = 0;
            bullets.splice(i, 1);
            continue;
          }
        }
      ...
      // Update the target practice bot if it exists
      if (targetBot) {
        targetBot.update();
      }
  ...
  function draw() {
    ...
    // Draw the target practice bot if it exists and has health.  We
    // render the bot as an orange circle and draw a health bar
    // representing its current health.  The health bar sits above
    // the bot and is proportional to the bot’s remaining health.
    if (targetBot && targetBot.health > 0) {
      const ex = targetBot.x * CELL_SIZE * scale + offsetX;
      const ey = targetBot.y * CELL_SIZE * scale + offsetY;
      const er = targetBot.radius * CELL_SIZE * scale;
      // Draw bot body
      ctx.beginPath();
      ctx.fillStyle = targetBot.color;
      ...
      const healthPct = targetBot.health / targetBot.maxHealth;
