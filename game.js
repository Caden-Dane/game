...
(function() {
    ...
    if (gameRunning && player) {
      ...
      for (const enemy of targetBots) {
        ...
        if (dist < enemy.radius + player.radius) {
          ...
          // enters a stun state for 2 seconds (120 frames) during
          ...
          if (!enemy.attackCooldown || enemy.attackCooldown <= 0) {
            ...
            enemy.stunFrames = 120;    // paralyse bot for 2 seconds
  ...
  function draw() {
    ...
    // screen coordinates and draw it before any obstacles or items.
    {
      ...
      // Use a subtle checkerboard pattern for the world floor rather
      // than a flat colour.  The pattern is tiled across the entire
      // world rectangle.
      ctx.fillStyle = worldPattern;
    ...
    obstacles.forEach((ob) => {
      // Use the precomputed obstacle texture pattern instead of a
      // flat grey.  Each obstacle shares the same pattern and
      // scales naturally when zooming.
      ctx.fillStyle = obstaclePattern;
      if (ob.type === 'rect') {
