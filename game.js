...
(function() {
  ...
  class Fruit {
    constructor() {
      ...
      // Larger radius differences so levels are visually distinct and easier
      // to see.  Increase sizes compared to earlier versions.
      const radii = {1: 0.4, 2: 0.6, 3: 0.8};
      this.radius = radii[this.level] || 0.4;
  ...
  class AmmoPickup {
    constructor() {
      ...
      // are clearly distinct from both fruit and one another, and larger
      // overall to improve visibility.
      const radii = {1: 0.35, 3: 0.5, 5: 0.7, 10: 0.9};
      this.radius = radii[this.amount] || 0.35;
    ...
    update() {
      ...
      if (this.target) {
        ...
        if (delta < this.speed * 0.01) {
          ...
          // We moved successfully; reset reroute rank so that the next time
          // we get stuck we start with the 2nd farthest item again
          this.rerouteRank = 2;
        }
