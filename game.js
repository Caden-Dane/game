...
(function() {
  ...
  // Texture patterns
  //
  // To add visual richness to the otherwise plain world and obstacles,
  // we pre‑generate small canvas patterns that will be tiled across
  // surfaces.  Patterns are created once here and reused each frame
  // via ctx.createPattern().  The world pattern is a subtle checker
  // made up of two very light greys; the obstacle pattern is a
  // cross‑hatch drawn over a mid‑tone grey background.  These
  // patterns provide depth without distracting from gameplay.
  // Create a checkerboard pattern for the world floor
  const worldPatternCanvas = document.createElement('canvas');
  worldPatternCanvas.width = 40;
  worldPatternCanvas.height = 40;
  const worldPctx = worldPatternCanvas.getContext('2d');
  worldPctx.fillStyle = '#f5f5f5';
  worldPctx.fillRect(0, 0, 40, 40);
  worldPctx.fillStyle = '#f0f0f0';
  worldPctx.fillRect(0, 0, 20, 20);
  worldPctx.fillRect(20, 20, 20, 20);
  const worldPattern = ctx.createPattern(worldPatternCanvas, 'repeat');
  // Create a cross‑hatch pattern for obstacles
  const obstaclePatternCanvas = document.createElement('canvas');
  obstaclePatternCanvas.width = 40;
  obstaclePatternCanvas.height = 40;
  const obstaclePctx = obstaclePatternCanvas.getContext('2d');
  obstaclePctx.fillStyle = '#bdbdbd';
  obstaclePctx.fillRect(0, 0, 40, 40);
  obstaclePctx.strokeStyle = '#9e9e9e';
  obstaclePctx.lineWidth = 2;
  obstaclePctx.beginPath();
  obstaclePctx.moveTo(0, 0);
  obstaclePctx.lineTo(40, 40);
  obstaclePctx.moveTo(40, 0);
  obstaclePctx.lineTo(0, 40);
  obstaclePctx.stroke();
  const obstaclePattern = ctx.createPattern(obstaclePatternCanvas, 'repeat');
  ...
  function startNewRound() {
    ...
    progressFrame = 0;
    if (progressBarEl) progressBarEl.style.width = '0%';
    // Increase the max health of existing enemies by 20 per round and
    // reset their health to the new maximum.  This ensures that
    // enemies spawn each round at full strength rather than
    // carrying over remaining health from the previous round.
  ...
  class Player {
    constructor(name) {
      ...
      this.upgradeIncrement = {
        bulletSpeed: 0.2,
        bulletRange: 0.2,
        // Increase speed upgrade effect from 15% to 20%.  Each speed
        // upgrade now multiplies the player's speed by an additional 0.2.
        speed: 0.20,
        health: 20,
        bulletDamage: 0.2,
      };
  ...
  class Fruit {
    constructor(level) {
      ...
      // Adjusted scoring so level one fruit award 3 points,
      // level two award 6 points and level three award 8 points.
      this.level = level;
      this.points = {1: 3, 2: 6, 3: 8}[level] || 3;
      ...
      if (this.target) {
        ...
        if (dist < this.radius + this.target.radius) {
          ...
          if (this.target.type === 'fruit') {
            ...
            // After round 5, double the points gained.
            let gained = this.target.points;
            if (roundLevel > 5) gained *= 2;
            player.score += gained;
            ...
            this.pointsCollected += gained;
            this.framesSinceLastCollection = 0;
          } else if (this.target.type === 'ammo') {
            // Award ammo to the player equal to the pickup amount.  After
            // round 5, double the amount gained.
            let ammoGained = this.target.amount;
            if (roundLevel > 5) ammoGained *= 2;
  ...
  class TargetBot {
    constructor() {
      ...
      // Stun timer used to paralyse the bot for a short period after
      // it deals damage to the player.  When stunFrames > 0 the bot
      // does not move or attack.  This gives the player a window to
      // escape.  Measured in update frames (60 per second).
      this.stunFrames = 0;
    ...
    update() {
      ...
      // If the bot is stunned, decrement the stun timer and do not
      // perform any movement or chasing.  Stunned bots still
      // decrement their attackCooldown but otherwise remain idle.
      if (this.stunFrames > 0) {
        this.stunFrames--;
        if (this.attackCooldown > 0) this.attackCooldown--;
        return;
      }
      // Always move at a constant speed equal to 90% of the player's
      // original base speed.  The bot's speed does not change
      // regardless of player upgrades or the current round.
      this.speed = (6 / 60) * 0.9;
    ...
    if (gameRunning && player) {
      ...
      for (let i = items.length - 1; i >= 0; i--) {
        ...
        if (dist < player.radius + it.radius) {
          ...
          items.splice(i, 1);
          if (it.type === 'fruit') {
            // Compute points gained.  Double the points if the game is
            // beyond round 5 (i.e. starting in round 6 and later).
            let gained = it.points;
            if (roundLevel > 5) gained *= 2;
            player.score += gained;
            ...
            // After round 5, double the ammunition gained.
            let ammoGained = it.amount;
            if (roundLevel > 5) ammoGained *= 2;
            player.ammo += ammoGained;
            ammoValue.textContent = 'Ammo: ' + player.ammo;
          }
      ...
      for (const enemy of targetBots) {
        ...
        if (dist < enemy.radius + player.radius) {
          // Bot touched the player.  If it isn't on cooldown it can
          // inflict damage.  After damaging the player, the bot
          // enters a stun state for 3 seconds (180 frames) during
          // which it will not move or attack.  A separate
          // attackCooldown of 1 second prevents constant damage on
          // consecutive frames.
          if (!enemy.attackCooldown || enemy.attackCooldown <= 0) {
            player.health -= 20;
            if (player.health < 0) player.health = 0;
            enemy.attackCooldown = 60; // 1 second cooldown for subsequent hits
            // Paralyse the bot for 3 seconds (180 frames) instead of 2 seconds.
            enemy.stunFrames = 180;
  ...
  function draw() {
    ...
    // screen coordinates and draw it before any obstacles or items.
    {
      ...
      // Fill the world area with a uniform light colour.  Any grid or
      // checker pattern has been removed to avoid visual boxes.
    ...
    obstacles.forEach((ob) => {
      // Draw obstacles with a solid mid‑grey.  Patterns and grid lines
      // have been removed to avoid visual clutter.
      ctx.fillStyle = '#999';
      if (ob.type === 'rect') {
    ...
    // Do not draw a border around the world.  Removing the outline
    // eliminates boxes or grid lines from the map.
    ...
    // Draw the current round number prominently at the top centre of
    // the screen.  Use a larger font and a darker colour so it
    // stands out against the light background.  We offset the text
    // slightly downward to avoid overlapping the browser UI.
    ctx.fillStyle = '#333';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'top';
    ctx.font = `${Math.floor(canvas.height * 0.05)}px sans-serif`;
    ctx.fillText('Round ' + roundLevel, canvas.width / 2, 10);

    // If the game is over due to the player dying, draw a big red
    // message in the centre of the screen.  This overlay informs the
    // player of their demise beyond the smaller status text.  Use
    // large text relative to canvas height.
    if (!gameRunning && player && player.health <= 0) {
      ctx.fillStyle = '#d32f2f';
