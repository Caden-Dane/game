    ...
    update() {
      ...
      if (touching) {
        ...
        if (this.contactFrames >= (this.redirectAttempts + 1) * threshold) {
          ...
          // As a creative solution for persistent obstruction, reposition the
          // bot near the player to free it from being wedged behind
          // obstacles.  This teleports the bot out of the stuck position and
          ...
          if (newTarget) {
            ...
            // Fallback: just clear target if none exists
            this.target = null;
          }
          // Reset contact frames so we don't immediately trigger again
          this.contactFrames = 0;
        }
      } else {
